...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        filtered_df['Success/Failure'] = filtered_df['class'].apply(lambda x: 'Success' if x==1 else 'Failure')
        success_counts = filtered_df['Success/Failure'].value_counts().reset_index()
        success_counts.columns = ['Success/Failure', 'count'] 
...
def get_scatterplot(entered_site, payload_range):
    if entered_site == 'ALL':
        ...
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        title='Correlation between Payload and Success at site ' + entered_site
    ...
    fig.update_layout(
        ...
        yaxis_tickvals=[0,1],
        yaxis_ticktext=['Failure', 'Success']
    )
    return fig
# Run the app
